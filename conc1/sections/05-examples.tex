...
Obviously, if two model categories are Quillen equivalent one is homotopy concrete if and only if the other is, and this requires no theorem whatsoever. So, as a consequence of \refbf{honoconc} every category Quillen equivalent to $\Top$ cannot be homotopy concrete. 
\end{example}
\begin{example}[Chain complexes]\label{complessi}
